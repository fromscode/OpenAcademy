# Essential API Endpoints for OpenAcademy Dashboard

This document outlines the essential API endpoints in `api.js` that are needed for the basic dashboard functionality of the OpenAcademy application.

## File Structure:
- **`api.js`** - Your main API file (USE THIS ONE)
- **`api-full.js`** - Backup of original comprehensive API (for reference only)
- **`API_ENDPOINTS.md`** - This documentation file
...
## Authentication Helper Functions (`authUtils`)
...
// Student login
const loginResult = await authAPI.student.login('student@example.com', 'password');

// Teacher login
const teacherResult = await authAPI.teacher.login('teacher@example.com', 'password');
```
...
const studentDashboard = await dashboardAPI.getStudentDashboard();

// Get teacher statistics
const teacherStats = await dashboardAPI.getTeacherStatistics();
```
...
// Grade a submission
await gradingAPI.teacher.gradeSubmission(submissionId, {
  grade: 85,
  maxGrade: 100,
  comments: 'Good work!'
});
...
// Add feedback
await gradingAPI.teacher.addFeedback(submissionId, 'Please review the formatting');
```
...
// Upload file as student
const formData = new FormData();
formData.append('file', fileObject);
formData.append('description', 'Assignment submission');

const uploadResult = await filesAPI.student.upload(formData);
```

## Notes

1. **IMPORTANT: Use only `api.js` in your application** - this contains all essential endpoints
2. All authenticated endpoints require a Bearer token in the Authorization header
3. File upload endpoints use FormData and don't set Content-Type header (browser sets it automatically)
4. All responses are JSON except for file downloads
5. Error handling is built into the `handleResponse` helper function
6. The original comprehensive API file is backed up as `api-full.js` for reference only

## Quick Start

Simply import from `api.js` in your components:

```javascript
import { authAPI, dashboardAPI, gradingAPI, filesAPI, basicAPI } from './services/api';
```

This essential API setup provides all the core functionality needed for:
- User authentication and authorization
- Dashboard data display
- Grading and feedback system
- File upload/download operations
- Basic profile and course management
